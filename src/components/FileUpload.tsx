        ...
        complete: (results) => {
          try {
            const transactions: ParsedTransaction[] = results.data.map((row: any, index: number) => {
              // Handle various CSV formats (common bank export formats)
              const date = row.Date || row.date || row.DATE || row['Transaction Date'] || row.timestamp
              const description = row.Description || row.description || row.DESCRIPTION || row.merchant || row.Merchant
              const amountStr = row.Amount || row.amount || row.AMOUNT || row.Debit || row.Credit || row.Value
              ...
              if (isNaN(amount)) {
                throw new Error(`Invalid amount in row ${index + 1}: ${amountStr}`)
              }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const transactions: ParsedTransaction[] = jsonData.map((row: any, index: number) => {
            // Similar parsing logic as CSV
            const date = row.Date || row.date || row.DATE || row['Transaction Date']
            const description = row.Description || row.description || row.DESCRIPTION || row.Merchant
            const amountStr = row.Amount || row.amount || row.AMOUNT || row.Debit || row.Credit
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Parsed {parsedCount} transactions from {uploadedFile.name}
              </p>
            </div>
            <button
              onClick={resetUpload}
